...
A typical interior page section — two-column layout with a heading, prose, and a CTA:
...
<section class="block">
  <div class="twocol">
    ...
    <div class="prose">
      ...
      <p>Every arrangement is built on a grid, pin frog, or water-bearing vessel — mechanics that keep flowers alive longer and leave nothing behind in Sierra watersheds.</p>
...
- Sections use `.block` (max-width 1500px) or `.block-narrow` (max-width 780px).
- Eyebrow always `.lab` above the heading.
- `<h2 class="disp">` for section headings; `<em>` for the forest-green accent word.
- Body copy wraps in `.prose` and uses `<p>` (with `class="lead"` for the opening line).
- One CTA per section, matched to the background (`.btn` on light, `.btn.ghost` on dark).
